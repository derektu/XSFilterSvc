...
class XSService {
     ...
     * @param {*} fields array of fields, 請傳入要查詢的欄位的TAID (不再支援欄位名稱) 
     * @param {*} startDate 查詢日期起點(yyyyMMdd)
     * @param {*} endDate 查詢日期終點(yyyyMMdd)
     * 
     * @returns {*} 回傳 array of {date, field1, field2, ..}
     */
    async queryFieldData(symbol, freq, fields, startDate, endDate) {
        ...
        var postXml = this._composeHistDataRequestXml(symbol, freq, fields, startDate, endDate);

        var version = 1;
        var action = 3;

        let xmldoc = await new XSRequest().postXmlRequst(this.server, version, action, postXml);
        return this._parseFieldData(xmldoc, fields);
    }

    _composeHistDataRequestXml(symbolID, freq, fields, startDate, endDate) {
        /*
         <?xml version='1.0' encoding='utf-8' ?>
         <Symbol ID='2330.TW' Freq='8' Fields='1;2;3' StartDate='yyyyMMdd' EndDate='yyyyMMdd' Order='desc'/>
         */
        let xml = '';
        let fs = fields.join(';');
        xml = xml + '<?xml version="1.0" encoding="UTF-8"?>';
        xml = xml + `<Symbol ID='${symbolID}' Freq='${freq}' Fields='${fs}' StartDate='${startDate}' EndDate='${endDate}' Order='desc'/>`
        return xml;
    }

    _parseFieldData(doc, fields) {
        /*
            xmldoc的格式如下:

            <Ret status="0">
                <Fields><![CDATA[ts;field1;field2;日期;內建欄位1;內建欄位2;內建欄位3;..]]></Fields>
                <Symbol ID="2330.TW" Freq="8" Count="全部筆數">
                    <val idx=".." ts=".." value="value1;value2;20110103;內建欄位數值1;內建欄位數值2;內建欄位數值3;.." />
                    <val idx=".." ts=".." value="value1;value2;20110102;內建欄位數值1;內建欄位數值2;內建欄位數值3;.." />
                    <..>
                </Symbol>
            </Ret>
     ...
     * 
     * @param {string} fieldsString server回傳的結果欄位名稱序列, e.g. ts;收盤價;成交量;日期;開盤價;收盤價;最低價;最高價;.., 這裡面會包含我們要查詢的欄位名稱/代碼, 以及"日期"等內建欄位
    ...
    _getFieldIndices(fieldsString, fields) {
        ...
        // 先找'日期' (日期的代碼=3)
        //
        let dateIndex = _.indexOf(retFields, '3');
        if (dateIndex < 0)
            throw `Cannot find 日期欄位(3)的位置`;    
